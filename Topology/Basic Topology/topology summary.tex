...
\begin{example}\label{top_euclidean}
	For a more interesting example, we can consider over the real numbers the topology $\tau = \{A \subseteq \reales\ :\ \forall x \in A\ \exists I \text{ open interval s.t. } x \in I \subseteq A\}$. This one is called the \textbf{euclidean topology} over the real numbers.
\end{example}
...
\begin{define}{Saturated set}{saturated_set}
	...
	\begin{equation*}
		S = p^{-1} (p(S)).
	\end{equation*}
	We denote the set of all saturated subsets of $X$ by $\sats(X)$ (whenever the EBR is clear).
\end{define}
...
\begin{define}{Separation axioms}{sep_axioms}
	...
	\begin{itemize}
		\item
		\textbf{T0} (or \textbf{Kolmogorov}) if for all $x,y \in X$ distinct there exist an open set $U$ such that one of $x,y$ is in $U$ and the other one isn't.
		
		\item
		\textbf{T1} (or \textbf{Frechet}) if for all $x,y \in X$ distinct there exist 2 open sets $U$ and $V$ such that $x \in U, y \not \in U$ and $x \not \in V, y \in V$.
		
		\item
		\textbf{T2} (or \textbf{Hausdorff}) if for all $x,y \in X$ distinct there exist 2 open sets $U$ and $V$ such that $x \in U$, $y \in V$ and $U \cap V = \emptyset$.
		...
		\item
		\textbf{T3} if it's regular and T1.
		...
		\item
		\textbf{T4} if it's normal and T1.
	\end{itemize}
\end{define}
...
\section{Rings of continuous real valued functions}
