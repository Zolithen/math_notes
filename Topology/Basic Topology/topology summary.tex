...
\begin{proof}
	By theorem \eqref{thm_compact_haus} $Y$ must be normal and so by Urysohn's lemma $Y$ is completely regular. Then $X$ is homeomorphic to $h(X)$, which is a subset of a completely regular space and so is completely regular.
\end{proof}
...
\begin{theorem}
	For every space $X$ there exists some space $tX$ such that $C(X) \cong C(tX)$ and $C^*(X) \cong C^*(tX)$.
\end{theorem}
