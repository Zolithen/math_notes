...
\begin{theorem}
	(AC) Let $R$ be a nonzero commutative ring. On the set $\maxspec(R)$ of maximal ideals of $R$ we define the sets $\mathcal{B}_f := \{\mathfrak{m} \in \maxspec(R)\ |\ f \not \in \mathfrak{m} \}$ for every $f \in R$. The collection $\{ \mathcal{B}_f\}_{f \in R}$ forms a base for a topology, the \textbf{Zariski topology}. This topology makes the space $\maxspec(R)$ compact.
\end{theorem}

\begin{proof}
	First of all we note that $\maxspec(R)$ is not empty thanks to Krull's theorem (every proper ideal of a nonzero commutative ring is contained in a maximal ideal). To show that the set $\{ \mathcal{B}_f\}_{f \in R}$ is a base, we use the definition  of a base \eqref{defn:base}. As all elements of $\maxspec(R)$ are proper ideals, we have $1 \not \in \mathfrak{m}$ for every $\mathfrak{m} \in \maxspec(R)$, so $\maxspec(R) = \mathcal{B}_{1}$. This proves the first condition of being a base. For the second condition, I leave the reader to prove $\mathcal{B}_{f} \cap \mathcal{B}_{g} = \mathcal{B}_{fg}$. This can be done easily using that every maximal ideal is prime.\\ 
	
	Now, to show that this space is compact we use the condition in exercise \eqref{exer:space_is_compact_base}. Suppose that $\{ \mathcal{B}_i \}_{i \in I}$ is a covering of $\maxspec(R)$ by basic sets, where $I \subseteq R$. If the ideal generated by $I$ was not equal to $R$, then by Krull's theorem it would be contained in a maximal ideal $\mathfrak{m}_I$ such that $I \subseteq \mathfrak{m}_I$ so that $\mathfrak{m}_I$ would not be in the union of $\{ \mathcal{B}_i \}_{i \in I}$, contradicting the fact that it was a basic cover. As such, the ideal generated by $I$ is equal to $R$ and so there exists some finite subset $J \subseteq I$ such that a finite sum $\Sigma_{j \in J} \alpha_j j = 1$ is achieved for some elements $\alpha_j \in R$. If an ideal $\mathfrak{m}$ were to not be found in $\cup_{j \in J} \mathcal{B}_j$ then $j \in \mathfrak{m}$ for all $j \in J$ and thus $1 \in \mathfrak{m}$, so $\mathfrak{m} \not \in \maxspec(R)$. $\{ \mathcal{B}_j \}_{j \in J}$ is then a finite subcover.
\end{proof}
Our objective is to use the Zariski topology on the ring $C^*(X)$, as it will be a compact Haussdorf topological space, on which we can easily embed $X$. This will provide a compactification of $X$, and we hope that building this topology in something close to $C^*(X)$ we'll be able to get something ``maximal'' in the sense discussed earlier.\\ 
Before studying $C^*(X)$, we need another result relating to the Zariski topology.
\begin{theorem}
