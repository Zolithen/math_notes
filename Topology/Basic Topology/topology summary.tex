...
Concatenation, as an algebraic operation, doesn't have many remarkable properties. Our objective is to make it a group operation. To make it so, we need to restrict ourselves to loop spaces (to make it an internal operation) and consider some kind of equivalence relation over it. To see why we need to consider an equivalence relation, we will see that concatenation generally doesn't have neutral elements nor inverses:
\begin{example}

\end{example}
And so we define path homotopy:
...
\begin{define}{Induced homomorphism}{induced_homomorphism}
	If $x \in X,\ y \in Y$ and $f: X \rightarrow Y$ is a continuous function such that $f(x) = f(y)$ then the application $\overline{f}: \pi_1(X, x) \rightarrow \pi_1(Y, y)$ defined by $\overline{f}([\alpha]) = [f \circ \alpha]$ is a group homomorphism that we call the homomorphism \textbf{induced} by $f$.
\end{define}
...
\begin{example}
	...
	\end{itemize}
	So $G$ is a strong deformation between the identity and $r$, so $A$ is a strong deformation retract of $X$ onto $A$. By the last theorem and $A$ and $X$ being path connected, the fundamental groups of $A$ and $X$ are isomorphic, so $X$ has trivial fundamental group.
\end{example}
