...
\begin{define}
	Una aplicación $f:I \rightarrow J$ con $I,J$ intervalos abiertos de números reales se dice $difeomorfismo$ si es biyectiva, $\cinfinito$ y su inversa también es $\cinfinito$.
\end{define}
...
TODO: y si $b \leq a$? se usa mas abajo en la naturalidad, parece que se usa que sea negativa asi que no hay que cambiar nada
\begin{define}
...
TODO: Esta mal? por ejemplo la lemniscata
\begin{prop}
...
\begin{define}
	Sea $S$ una superficie regular y $p \in S$. Decimos que $v \in \reales^3$ es un \textbf{vector tangente} en $p$ a $S$ si existe una curva regular $\alpha: (-\varepsilon, \varepsilon) \rightarrow S$ tal que $\alpha(0) = p$ y $\alpha' (0) = v$. El conjunto de todos los vectores tangentes en un punto $p \in S$ a $S$ se denota como $T_p S$.
\end{define}

\begin{prop}
	$T_pS$ es un subespacio vectorial de $\reales^3$ de dimensión $2$ (es decir, un plano vectorial). Además, para cualquier parametrización $X: U \rightarrow S$ en $p$ se tiene $\text{Im}(dX_q) = T_pS$ para $X(q) = p$ y $\{ X_u, X_v\}$ es una base de $T_pS$.
\end{prop}
...
\begin{define}
	Sea $S$ una superficie regular y $p \in S$. Sea $\langle \cdot, \cdot \rangle_p$ el producto escalar heredado en $T_pS$ del espacio euclideo estándar $\reales^3$. La \textbf{primera forma fundamental} (1ª FF) es la forma cuadrática en $T_pS$ dada por:
	\begin{equation*}
		I_p(v) = \langle v, v\rangle_p.
	\end{equation*}
\end{define}

\begin{prop}
	Dada una parametrizacion $X: U \rightarrow S$ en $p$, los coeficientes de $I_p$ en la base $\{ X_u, X_v\}$ son:
	\begin{equation*}
		\left(
		\begin{array}{cc}
			E & F \\ 
